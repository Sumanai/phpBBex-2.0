...
if ($mode == 'edit' && !$auth->acl_get('m_edit', $forum_id))
{
	...
	$s_cannot_edit_time = !$auth->acl_get('u_ignoreedittime') && !($post_data['topic_first_post_id'] == $post_id && $auth->acl_get('u_ignorefpedittime')) && $config['edit_time'] && $post_data['post_time'] <= time() - ($config['edit_time'] * 60);
...
$uninit = array('post_attachment' => 0, 'poster_id' => $user->data['user_id'], 'enable_magic_url' => 0, 'topic_status' => 0, 'topic_type' => POST_NORMAL, 'topic_priority' => 0, 'post_subject' => '', 'topic_title' => '', 'post_time' => 0, 'post_edit_reason' => '', 'notify_set' => 0);
...
if ($submit || $preview || $refresh)
{
	...
	$poll_delete		= isset($_POST['poll_title']) && trim(request_var('poll_title', '')) === '' && isset($_POST['poll_option_text']) && trim(request_var('poll_option_text', '')) === '' || isset($_POST['poll_delete']);
	$poll_reset			= (isset($_POST['poll_reset'])) ? true : false;
	...
	if (!sizeof($error) && $submit)
	{
		if ($submit)
		{
			...
			*/
			...
			{
				$perm_show_unshow = ($auth->acl_get('m_lock', $forum_id) || ($auth->acl_get('f_user_lock', $forum_id) && $user->data['is_registered'] && !empty($data['topic_poster']) && $user->data['user_id'] == $data['topic_poster'])) ? true : false;
