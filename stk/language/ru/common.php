...
$lang = array_merge($lang, array(
	...
	'CAT_ADMIN_EXPLAIN'					=> 'Средства администрирования могут быть использованы администратором для управления конкретными аспектами форума и решать общие проблемы.',
	...
	'CAT_ERK_EXPLAIN'					=> 'Аварийный ремонтный комплект включает отдельные инструменты STK, которые выполняют проверку и помогают обнаружить проблемы, мешающие нормальной работе вашего форума. Нажмите <a href="%s"><b>здесь</b></a> для выполнения АРК.',
	'CAT_MAIN'							=> 'Главная',
	'CAT_MAIN_EXPLAIN'					=> 'Support Toolkit, или STK представляет собой программный пакет, который может быть использован для устранения проблем, связанных с работой phpBBex 2.x. Инструментарий является второй <b>Панелью администратора</b>, предоставляющей администратору доступ к функциям, используемым для разрешения различных проблем неправильной работы форума.',
	'CAT_SUPPORT'						=> 'Инструменты поддержки',
	'CAT_SUPPORT_EXPLAIN'				=> 'Средства поддержки используются для фиксации проблем или восстановления отдельных частей установки phpBBex 2.x.',
	'CAT_USERGROUP'						=> 'Средства пользователи и группы',
	'CAT_USERGROUP_EXPLAIN'				=> 'Средства пользователи и группы могут быть использованы для управления пользователями и группами, такими способами, которые не доступны в стандартной установке phpBBex 2.x.',
	...
	'GEN_PASS_FILE_EXPLAIN'				=> 'Если вы не можете авторизироваться на форуме, вы можете использовать метод авторизации через STK. Для этого вы должны <a href="%s"><strong>сгенерировать</strong></a> новый файл пароля.',

	'INCORRECT_CLASS'					=> 'Некорректный класс: stk/tools/%1$s.%2$s',
	'INCORRECT_PASSWORD'				=> 'Пароль некорректен',
	'INCORRECT_PHPBB_VERSION'			=> 'Ваша версия phpBB несовместима с данным STK. Убедитесь, что вы используете версию phpBB %1$s или выше для запуска этого инструмента.',
	...
	'PLUGIN_INCOMPATIBLE_PHPBB_VERSION'	=> 'Эта версия STK несовместима с используемой вами версией phpBBex',
	...
	'ANONYMOUS_MISSING'					=> 'Support Toolkit определил, что учетная запись Гостя была удалена из базы данных, поэтому ваш форум не может нормально функционировать.<br />
	...
	'ERK_ISSUE_FOUND'					=> '“Аварийный ремонтный комплект” (АРК) как часть STK проверил файлы phpBBex и определил, что некоторые из них содержат недопустимый контент, который потенциально может нарушить работу форума. STK попытался решить эту проблему и создал пакет с исправленными файлами <em>(резервные копии можно найти в каталоге <c>store/bom_sniffer_backup/</c>)</em>. Этот пакет сохранен в каталоге <c>store/bom_sniffer/</c> . Чтобы применить изменения <strong>переместите</strong> эти файлы из каталога “store” в соответсвующие каталоги phpBBex и перезапустите АРК. Инструмент снова проверит файлы и перенаправит вас в STK, если не будет обнаружено никаких недостатков.<br /><br /><strong style="color: #ff0000;">Прежде чем переместить сгенерированные файлы, убедитесь, что они не содержат ошибок!</strong> В случае сомнений обратитесь за помощью на <a href="http://www.phpbb.com/community/viewforum.php?f=46">форум поддержки</a>.',
	'ERK_STORE_WRITE'					=> 'Для работы перехватчика метки BOM требуется, чтобы каталог <c>store</c> существовал и допускала запись!',
	'ERK_REMOVE_DIR'					=> 'STK пытался удалить каталог с восстановлеными файлами, но не смог этого сделать. Для корректной работы этого инструмента каталог\'<c>%s</c>\' должен быть удалён с сервера. Удалите этот каталог вручную и перезапустите АРК.',
	'BOM_SNIFFER_WRITABLE'				=> 'Для работы перехватчика метки BOM требуется, чтобы каталог ' . STK_ROOT_PATH . 'cache существовал и допускал запись!',
	'STK_FATAL_ERROR'					=> '<strong style="color: #ff0000;">Произошла неустранимая ошибка.</strong><br /><br />
											Support Toolkit включает в себя Аварийный восстановительный комплект (АРК) - инструмент, созданный для решения некоторых ошибок, которые мешают функционировать phpBBex.<br />
	...
	'CONFIG_REPAIR_NO_TABLES'			=> 'В указанной базе данных не найдено таблиц phpBBex с указанным префиксом.',
	...
	'CONFIG_REPAIR_WRITE_ERROR'			=> '<strong style="color: #ff0000;">ОШИБКА: Не удалось записать конфигурационный файл.</strong><br />Пожалуйста, скопируйте приведенный ниже текст, поместите его в файл с именем config.php, и поместите его в корневой каталог вашего форума.<br /><br />',

	'CONFIG_LIST'						=> 'Параметры конфигурации',
	'CONFIG_LIST_EXPLAIN'				=> 'Здесь вы можете просмотреть и изменить параметры конфигурации.',
	'CLOSE'								=> 'Закрыть',
	'UPDATES_AVAILABLE'					=> 'Ваша версия phpBB не самая последняя. Доступная для обновления версия %1$s<br />Перейдите по этой ссылке <a href="%2$s" target="_blank" />%2$s</a> на объявление о выпуске последней версии,<br />которое содержит дополнительную информацию, а также инструкции по обновлению phpBBex',
	'VERSIONCHECK_FAIL'					=> 'Не удалось получить сведения о последней версии phpBBex.',
