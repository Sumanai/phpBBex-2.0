...
$lang = array_merge($lang, array(
	...
	'BOARD_NOT_INSTALLED'			=> 'phpBBex не установлен',
	'BOARD_NOT_INSTALLED_EXPLAIN'	=> 'Для работы единой системы конвертирования требуется установленная по умолчанию версия phpBBex, пожалуйста, <a href="%s">сначала перейдите к установке phpBBex</a>.',
	...
	'COLLIDING_USERNAMES_FOUND'	=> 'На вашем старом форуме найдены конфликтующие имена пользователей. Для завершения преобразования удалите или переименуйте этих пользователей так, чтобы каждому очищенному имени соответствовал только один пользователь старого форума.',
	...
	'CONFIG_PHPBB_EMPTY'		=> 'Переменная конфигурации phpBBex для «%s» пуста.',
	...
	'CONVERT_COMPLETE_EXPLAIN'	=> 'Вы успешно сконвертировали ваш форум в формат phpBBex 2.0. Вы можете <a href="../">войти на форум</a>. Убедитесь, что все настройки успешно перенесены, прежде чем запустите форум, удалив каталог install. Помните, что вспомогательную информацию по использованию phpBBex можно получить в интернете, изучив <a href="https://www.phpbb.com/support/docs/en/3.1/ug/">документацию</a>, а также на <a href="https://www.phpbb.com/phpBB/viewforum.php?f=466">форуме поддержки phpBB Group</a> и форумах официальной русской поддержки <a href="http://www.phpbbguru.net">phpBB Guru</a>.',
	'CONVERT_INTRO'				=> 'Вас приветствует система конвертирования phpBBex',
	'CONVERT_INTRO_BODY'		=> 'Здесь вы имеете возможность импортировать данные из других (установленных) типов форумов. В списке ниже содержатся доступные в настоящее время модули для конвертирования. Если в нём отсутствует форум, которую вы хотели бы конвертировать, пожалуйста, посетите наш веб-сайт, на котором могут быть доступны для скачивания дополнительные модули.',
	...
	'CONV_ERROR_ATTACH_FTP_DIR'			=> 'На старом форуме включена загрузка вложений по FTP. Отключите эту функцию и убедитесь в правильности указанного каталога для загрузки, затем скопируйте все файлы вложений в этот новый каталог, доступный через интернет. После этого перезапустите конвертер.',
	...
	'CONV_ERROR_GET_CONFIG'				=> 'Не удалось получить информацию о конфигурации форума.',
	...
	'CONV_OPTIONS_BODY'					=> 'На этой странице вам предлагается ввести данные, необходимые для доступа к исходному форуму. Укажите информацию о базе данных вашего старого форума; конвертер не будет вносить в неё никаких изменений. Для обеспечения полноценной конвертации исходный форум следует отключить.',
	...
	'DB_UPDATE_NOT_SUPPORTED'	=> 'К сожалению, данный сценарий не поддерживает обновление с версий phpBBex до «%1$s». Установленная вами версия — «%2$s». Обновите предыдущую версию перед выполнением этого сценария. Помощь по этой проблеме можно найти на форумах поддержки на сайте phpBB.com.',
	'DB_USERNAME'				=> 'Имя пользователя БД',
	'DB_TEST'					=> 'Проверка подключения',
	'DEFAULT_LANG'				=> 'Язык форума по умолчанию',
	'DEFAULT_PREFIX_IS'			=> 'Конвертеру не удалось найти таблицы phpBB2 с указанным префиксом. Убедитесь, что вы ввели правильные данные о старом форуме. У таблиц форума %1$s префикс <strong>%2$s</strong>.',
	'DEV_NO_TEST_FILE'			=> 'В конвертере не указано значение переменной test_file. Если вы пользователь этого конвертера, то сообщите об этой ошибке его автору, поскольку она не должна возникать. Если вы автор конвертера, то вы должны задать имя существующего файла конвертируемого форума для проверки правильности пути к ней.',
	...
	'DL_CONFIG_EXPLAIN'			=> 'Вы можете целиком скачать файл config.php к себе на компьютер. Затем вам будет нужно вручную закачать этот файл в корневой каталог phpBBex, заменив исходный config.php. Помните, что файл должен быть закачан на сервер в формате ASCII (если вы не знаете как этого добиться, обратитесь к документации вашего FTP-клиента). После того, как закачаете config.php, щелкните «Готово» для перехода к следующему шагу.',
	'DL_DOWNLOAD'				=> 'Скачать',
	'DONE'						=> 'Готово',
	...
	'FORUM_ADDRESS_EXPLAIN'		=> 'URL старого форума, например, <samp>http://www.example.com/phpBB2/</samp>. Если вы заполните это поле, то во всех сообщениях, ЛС и подписях этот адрес будет заменён на адрес нового форума.',
	'FORUM_PATH'				=> 'Путь к форуму',
	'FORUM_PATH_EXPLAIN'		=> '<strong>Относительный</strong> путь к старому форуму от <strong>корневого каталога установленного phpBBex</strong>.',
	'FOUND'						=> 'Найден',
	...
	'FTP_PATH'					=> 'Путь FTP',
	'FTP_PATH_EXPLAIN'			=> 'Путь от корневого каталога FTP до корневого каталога phpBBex, например, <samp>htdocs/phpBBex/</samp>',
	'FTP_UPLOAD'				=> 'Закачать',
	...
	'INSTALL_CONGRATS_EXPLAIN'	=> '
		<p>Вы успешно установили phpBBex %1$s. Теперь вы можете начать использование только что установленного форума!</p>
		<p>Нажав на кнопку ниже, вы попадёте в панель администратора. Ознакомьтесь с имеющимися пунктами меню и настройками. Освоение имеющихся возможностей phpBBex может занять некоторое время. Помните о доступной в интернете <a href="http://www.phpbb.com/support/documentation/3.0/">документации на phpbb.com</a> (на английском) и <a href="http://phpbbex.com/">форуме русскоязычной поддержки phpBBex</a>.</p><p><strong>Удалите, переместите или переименуйте каталог «install» перед использованием форума. В противном случае будет доступна только панель администратора.</strong>',
	'INSTALL_INTRO'				=> 'Добро пожаловать в установку phpBBex!',

	'INSTALL_INTRO_BODY'		=> 'phpBBex (phpBB extended) — это новый и динамично развивающийся форк phpBB 3. Проект был создан по причине того, что оригинальный phpBB 3 развивается очень медленно, и с момента выхода в 2007 году в нём практически не было видимых изменений. Большинство проделанных изменений не существует в виде модов. Практически все установленные моды (из существующих вне phpBBex) были сильно переработаны: исправлены ошибки, код адаптирован под последние версии phpBB. Если phpBBex без дополнительных модификаций вас устраивает больше, чем стандартный phpBB 3 — несомненно, phpBBex станет для вас верным помощником. <a target="_blank" href="http://phpbbex.com/forum/viewtopic.php?t=2">Подробнее...</a>

	<p>Для установки phpBBex вам понадобятся следующие реквизиты доступа к базе данных MySQL:
	<ul>
		<li>Имя сервера БД или DSN — адрес сервера базы данных MySQL;</li>
	...
	<p>Смотрите также:
	<ul>
		<li><a target="_blank" href="http://phpbbex.com/">Официальный сайт phpBBex</a></li>
		<li><a target="_blank" href="http://phpbbex.com/forum/viewtopic.php?t=2">Что нового в phpBBex?</a></li>
		<li><a target="_blank" href="http://phpbbex.com/forum/viewtopic.php?t=8">FAQ</a></li>
		<li><a target="_blank" href="http://phpbbex.com/forum/viewforum.php?f=12">Бесплатная поддержка</a></li>
		<li><a target="_blank" href="http://phpbbex.com/forum/viewtopic.php?t=7">Конвертация базы из phpBB 3 в phpBBex</a></li>
		<li><a target="_blank" href="http://phpbbex.com/forum/feed.php?mode=topics">RSS с новостями проекта</a></li>
	</ul>',
	...
	'INST_ERR_DB_NO_POSTGRES'	=> 'Выбранная база данных создана не с кодировкой <var>UNICODE</var> или <var>UTF8</var>. Попробуйте установить форум в базу данных с кодировкой <var>UNICODE</var> или <var>UTF8</var>.',
	...
	'NO_LOCATION'				=> 'Не удалось найти приложение. Если вы знаете, что Imagemagick установлен, то вы можете указать путь к нему после установки форума в панели администратора.',
	...
	'PHP_JSON_SUPPORT_EXPLAIN'		=> '<strong>Необходима</strong>. Для правильной работы phpBBex необходимо наличие расширения PHP JSON.',
	...
	'PHP_REGISTER_GLOBALS_EXPLAIN'	=> 'phpBBex будет работать, даже если параметр register_globals включён. Но, по возможности, его рекомендуется отключить из соображений безопасности.',
	'PHP_SAFE_MODE'					=> 'Безопасный режим (Safe mode)',
	'PHP_SETTINGS'					=> 'Версия и настройки PHP',
	'PHP_SETTINGS_EXPLAIN'			=> 'Для установки phpBBex <strong>требуется</strong> версия PHP не ниже 5.3.3. Если ниже написано <var>Безопасный режим (Safe mode)</var>, то PHP работает в этом режиме. Это наложит некоторые ограничения на удалённое администрирование и аналогичные функции.',
	...
	'SCRIPT_PATH_EXPLAIN'		=> 'Путь к каталогу, содержащему phpBBex, относительно корня сайта. Например, <samp>/phpBBex</samp>.',
	'SELECT_LANG'				=> 'Выберите язык',
	'SERVER_CONFIG'				=> 'Конфигурация сервера',
	'SEARCH_INDEX_UNCONVERTED'	=> 'Поисковый индекс не был сконвертирован',
	'SEARCH_INDEX_UNCONVERTED_EXPLAIN'	=> 'Ваш старый поисковый индекс не был сконвертирован. Любой поисковый запрос вернёт пустой результат. Чтобы создать новый поисковый индекс, перейдите в панель администратора, выберите подраздел «Обслуживание» и в нём пункт «Поисковый индекс».',
	'SELECT_FORUM_GA'			=> 'В phpBBex важные темы привязаны к форумам. Выберите форум для данной важной темы (тема может быть перенесена в другой форум позже):',
	'SOFTWARE'					=> 'Тип форума',
	...
	'TABLE_PREFIX_SAME'			=> 'Префикс таблиц должен быть тем же, который используется конвертируемым форумом.<br />» Указанный префикс — %s.',
...
// Updater
$lang = array_merge($lang, array(
	'ALL_FILES_UP_TO_DATE'		=> 'Все файлы соответствуют самой последней версии phpBBex.',
	...
	'CHECK_FILES_UP_TO_DATE'		=> 'Согласно информации, находящейся в базе данных, ваша версия phpBBex самая новая. Вы можете приступить к сравнению файлов, чтобы убедиться в том, что все файлы действительно соответствуют самой последней версии phpBBex.',
	...
	'DOWNLOAD_UPDATE_METHOD_EXPLAIN'	=> 'Скачав архив, распакуйте его. В нём вы найдёте изменённые файлы, которые нужно закачать в корень phpBBex, при этом каждый файл должен оказаться в соответствующем ему каталоге. После этого снова проведите сравнение файлов, нажав отвечающую за это кнопку ниже.',

	'EDIT_USERNAME'	=> 'Изменить имя пользователя',
	'ERROR'			=> 'Ошибка',
	'EVERYTHING_UP_TO_DATE'		=> 'Всё обновлено до самой последней версии phpBBex. Теперь <a href="%1$s">войдите на форум</a> и проверьте, что всё работает нормально. Не забудьте удалить, переименовать или переместить каталог «install»! Пожалуйста, отправьте нам обновлённую информацию о вашем сервере и конфигурации форума с помощью страницы <a href="%2$s">Статистические данные</a> в панели администратора.',
	...
	'FILES_CONFLICT_EXPLAIN'		=> 'Следующие файлы модифицированы и не соответствуют оригинальным файлам старой версии. phpBBex определил, что эти файлы будут создавать конфликт при объединении их с новыми версиями. Изучите конфликтные места и попытайтесь исправить их вручную, либо продолжите обновление, выбрав подходящий тип объединения файлов. Если вы будете разрешать конфликты вручную, то снова проведите сравнение файлов после внесения в них изменений. Для каждого файла в отдельности вы можете выбрать предпочтительный метод объединения с новой версией. Результатом работы первого метода будет файл, в котором отброшены конфликтующие строки из вашего старого файла, во втором методе будут отброшены конфликтующие строки из новой версии.',
	...
	'FILES_NEW_EXPLAIN'				=> 'Следующих файлов нет в вашей версии phpBBex. Эти файлы будут добавлены.',
	...
	'FILES_NOT_MODIFIED_EXPLAIN'	=> 'Следующие файлы не были изменены и представляют собой оригинальные файлы phpBBex той версии, которую вы хотите обновить.',
	...
	'INCOMPATIBLE_UPDATE_FILES'		=> 'Найденные файлы обновления не подходят для установленной версии phpBBex. Ваша версия — %1$s, а файл обновления предназначен для перехода с версии %2$s к версии %3$s.',
	...
	'LOGIN_UPDATE_EXPLAIN'	=> 'Для того, чтобы обновить версию phpBBex, сперва необходимо войти на форум.',

	'MAPPING_FILE_STRUCTURE'	=> 'Для облегчения закачки файлов здесь указаны соответствия расположений файлов в архиве и в вашей версии phpBBex.',
	...
	'NO_AUTH_UPDATE'				=> 'У вас нет прав на выполнение обновления phpBBex',
	'NO_ERRORS'						=> 'Без ошибок',
	'NO_UPDATE_FILES'				=> 'Следующие файлы не будут обновлены',
	'NO_UPDATE_FILES_EXPLAIN'		=> 'Следующие файлы появились или были изменены в последней версии, но каталог, в которой они должны находиться, не найден в текущей версии phpBBex. Присутствие в списке файлов, не относящихся к каталогу языков (language/) или стилей (styles/), может означать, что вы изменили структуру каталогов и обновление будет неполным.',
	'NO_UPDATE_FILES_OUTDATED'		=> 'Не найден каталог с обновлениями. Убедитесь, что вы закачали соответствующие файлы на сервер.<br /><br />Ваша версия phpBBex %1$s <strong>не</strong> самая последняя, для неё доступны обновления. Зайдите на страницу <a href="https://www.phpbb.com/downloads/" rel="external">https://www.phpbb.com/downloads/</a>, чтобы скачать нужный вам пакет обновления с версии %2$s до версии %3$s.',
	'NO_UPDATE_FILES_UP_TO_DATE'	=> 'Ваша версия phpBBex самая последняя. Нет необходимости запускать программу обновления. Если вы хотите провести проверку целостности файлов, удостоверьтесь, что закачали подходящие файлы обновления.',
	...
	'OLD_UPDATE_FILES'		=> 'Имеющиеся файлы обновления phpBBex устарели. Они предназначены для обновления с версии %1$s до версии %2$s, но последняя версия phpBBex — %3$s.',
	...
	'UPDATE_INSTRUCTIONS'			=> '
		...
		<h1>Как обновить свою версию phpBBex с помощью пакета автоматического обновления</h1>

		<p>Рекомендованный здесь способ обновления phpBBex пригоден только для пакета автоматического обновления. Вы также можете произвести обновление, используя методы, указанные в документе INSTALL.html. Для автоматического обновления нужно выполнить следующие шаги:</p>
		...
		<ul style="margin-left: 20px; font-size: 1.1em;">
			...
			<li>Закачать разархивированные каталоги «install» и «vendor» в корневой каталог phpBBex (туда же, где находится файл config.php).<br /><br /></li>
		</ul>

		<p>После этого форум станет недоступен для обычных пользователей, из-за того, что теперь присутствует закачанный вами каталог install.<br /><br />
		<strong><a href="%1$s" title="%1$s">Теперь вы можете начать процесс обновления, перейдя в каталог install</a>.</strong><br />
		<br />
	...
	'UPDATE_SUCCESS_EXPLAIN'		=> 'Все файлы успешно обновлены. Следующий шаг включает в себя повторную проверку соответствия файлов последней версии phpBBex.',
	...
	'VERSION_CHECK_EXPLAIN'			=> 'Проверяет, что текущая версия phpBBex самая последняя.',
	'VERSION_NOT_UP_TO_DATE'		=> 'Ваша версия phpBBex не самая последняя. Продолжайте процесс обновления.',
	'VERSION_NOT_UP_TO_DATE_ACP'	=> 'Ваша версия phpBBex не самая последняя.<br />Ниже вы найдёте ссылку на объявление о выпуске последней версии, которое содержит дополнительную информацию, а также инструкции по обновлению phpBBex.',
	'VERSION_NOT_UP_TO_DATE_TITLE'	=> 'Ваша версия phpBBex не самая последняя.',
	'VERSION_UP_TO_DATE'			=> 'Ваша версия phpBBex самая последняя. На данный момент для неё нет обновлений. Тем не менее вы можете перейти к проверке файлов на соответствие последней версии.',
	'VERSION_UP_TO_DATE_ACP'		=> 'Ваша версия phpBBex самая последняя. На данный момент для неё нет обновлений.',
